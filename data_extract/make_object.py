...
class Maker():
    ...
    def make_player(self, page):
        ...
        player.name = face[1].get_text().strip()
        player.club = face[2].get_text().strip()
        player.nation = face[3].get_text().strip()
        player.league = face[4].get_text().strip()
        player.sf = int(face[5].get_text().strip())
        player.wf = int(face[6].get_text().strip())
        player.ir = int(face[7].get_text().strip())
        player.foot = face[8].get_text().strip()
        player.height = float(face[9].get_text().split("|")[0].strip("cm "))
        player.weight = float(face[10].get_text().strip(""))
        player.version = face[11].get_text().strip()
        player.def_wr = face[12].get_text().strip()
        player.att_wr = face[13].get_text().strip()
        player.added_on = datetime.strptime(face[14].get_text().strip()[2:], "%y-%m-%d")
        player.real_face = face[16].get_text().strip()=="icon-checkmark text-success"
        player.body_type = face[17].get_text().strip()
        player.age = int(face[18].get_text().strip(" years old \n\r"))
